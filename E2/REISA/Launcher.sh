...
# RUNNING
echo -e "Executing $(sbatch --parsable -N $NNODES --mincpus=40 --partition cpu_short --ntasks=$NPROC Script.sh $SIMUNODES $MPI_PER_NODE $CPUS_PER_WORKER $9 ${10}) in $MAIN_DIR/$OUTPUT/" >> ../../../../jobs.log
cd $MAIN_DIR