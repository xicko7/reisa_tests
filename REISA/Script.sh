...
for ((; i < $SLURM_JOB_NUM_NODES; i++)); do
    ...
    srun  --nodes=1 --ntasks=1 --relative=$i --cpus-per-task=$(($MPI_PER_NODE+2)) --mem=128G \
      ray start --address $RAY_ADDRESS --block --num-cpus=$((1+$IN_SITU_RESOURCES)) --resources="{\"actor\": 1, \"compute\": ${IN_SITU_RESOURCES}}" --object-store-memory $((96*10**9))  1>/dev/null 2>&1 &
done
